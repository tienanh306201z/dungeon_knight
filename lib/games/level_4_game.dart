...
import 'package:dungeon_knight/games/level_5_game.dart';
...
class Level4Game extends StatefulWidget {
  static bool useJoystick = true;

  const Level4Game({super.key});

  @override
  State<Level4Game> createState() => _Level4GameState();
}

class _Level4GameState extends State<Level4Game> implements GameListener {
  ...
  Widget build(BuildContext context) {
      ...
      'ladder': (p) => Ladder(p.position, p.size, canGo, () {
            if (!canGo) {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(builder: (context) => const Level5Game()),
              );
            }
          }),
    ...
    double tileX = indexMap == 0 ? 3 : 6;
    double tileY = indexMap == 0 ? 3 : 22;
    ...
    return Material(
      ...
      child: BonfireWidget(
        ...
        joystick: joystick,
        player: Knight(
          Vector2(tileX * tileSize, tileY * tileSize),
          'Hiệp sĩ',
        ),
        ...
        map: WorldMapByTiled(
          'tiled/level_4_map_$indexMap.json',
